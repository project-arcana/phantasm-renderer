#include "PrimitivePipeline.hh"

#include <clean-core/assert.hh>

#include "Frame.hh"

pr::PrimitivePipeline::PrimitivePipeline(pr::Frame* parent, phi::handle::pipeline_state pso) : mParent(parent), mPSO(pso)
{
    mDrawCmd.pipeline_state = mPSO;
}

void pr::PrimitivePipeline::draw(unsigned num_vertices)
{
...
void pr::PrimitivePipeline::draw(const pr::buffer& vertex_buffer)
{
...
void pr::PrimitivePipeline::draw_indexed(const pr::buffer& vertex_buffer, const pr::buffer& index_buffer)
{
...
void pr::PrimitivePipeline::add_argument(const pr::buffer& constant_buffer, uint32_t constant_buffer_offset)
{
    mDrawCmd.add_shader_arg(constant_buffer._resource.data.handle, constant_buffer_offset);
}

void pr::PrimitivePipeline::pop_argument() { mDrawCmd.shader_arguments.pop_back(); }

void pr::PrimitivePipeline::clear_arguments() { mDrawCmd.shader_arguments.clear(); }
