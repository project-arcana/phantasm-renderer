...
#define PR_VK_ASSERT_SUCCESS(_val_)                                                                    \
    ...
    (CC_UNLIKELY(_val_ != VK_SUCCESS) ? ::pr::backend::vk::detail::vk_verify_failure_handler(_val_, #_val_ " is not VK_SUCCESS", __FILE__, __LINE__) : void(0))
...
#define PR_VK_ASSERT_NONERROR(_val_)                                                                    \
    ...
    (CC_UNLIKELY(_val_ < VK_SUCCESS) ? ::pr::backend::vk::detail::vk_verify_failure_handler(_val_, #_val_ " is an error value", __FILE__, __LINE__) : void(0))
...
#define PR_VK_VERIFY_SUCCESS(_expr_)                                                                    \
    static_assert(PR_IS_PRVALUE_EXPRESSION(_expr_), "Use PR_VK_ASSERT_SUCCESS for lvalue expressions"); \
    do                                                                                                  \
    {                                                                                                   \
...
#define PR_VK_VERIFY_NONERROR(_expr_)                                                                    \
    static_assert(PR_IS_PRVALUE_EXPRESSION(_expr_), "Use PR_VK_ASSERT_NONERROR for lvalue expressions"); \
    do                                                                                                   \
    {                                                                                                    \
        ::VkResult const op_res = (_expr_);                                                              \
        if (CC_UNLIKELY(op_res < VK_SUCCESS))                                                            \
