...
public:
    ...
    */
    ...
    void InitBlockAllocation(VmaDeviceMemoryBlock* block, VkDeviceSize offset, VkDeviceSize alignment, VkDeviceSize size, VmaSuballocationType suballocationType, bool mapped, bool canBecomeLost)
    {
            ...
            if (pBestRequestBlock != VMA_NULL)
            {
                ...
                if (pBestRequestBlock->m_pMetadata->MakeRequestedAllocationsLost(currentFrameIndex, m_FrameInUseCount, &bestRequest))
                {
                    ...
                    *pAllocation = m_hAllocator->m_AllocationObjectAllocator.Allocate();
                    ...
                    (*pAllocation)
                        ->InitBlockAllocation(pBestRequestBlock, bestRequest.offset, alignment, size, suballocType, mapped,
...
VkResult VmaAllocator_T::Init(const VmaAllocatorCreateInfo* pCreateInfo)
{
    ...
    if (pCreateInfo->pRecordSettings != VMA_NULL && !VmaStrIsEmpty(pCreateInfo->pRecordSettings->pFilePath))
    {
        ...
        m_pRecorder->WriteConfiguration(m_PhysicalDeviceProperties, m_MemProps, m_UseKhrDedicatedAllocation, m_UseKhrBindMemory2);
                                              ...
                                              VmaAllocation* pAllocations)
{
    ...
    if ((finalCreateInfo.flags & VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT) != 0)
    {
        ...
        {
            return AllocateDedicatedMemory(size, suballocType, memTypeIndex, (finalCreateInfo.flags & VMA_ALLOCATION_CREATE_MAPPED_BIT) != 0,
    ...
    else
    {
        ...
        {
            res = AllocateDedicatedMemory(size, suballocType, memTypeIndex, (finalCreateInfo.flags & VMA_ALLOCATION_CREATE_MAPPED_BIT) != 0,
                                          (finalCreateInfo.flags & VMA_ALLOCATION_CREATE_USER_DATA_COPY_STRING_BIT) != 0, finalCreateInfo.pUserData,
                                          dedicatedBuffer, dedicatedImage, allocationCount, pAllocations);
            if (res == VK_SUCCESS)
            {
...
void VmaAllocator_T::SetCurrentFrameIndex(uint32_t frameIndex) { m_CurrentFrameIndex.store(frameIndex); }
