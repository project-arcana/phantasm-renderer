...
pr::backend::vk::image pr::backend::vk::create_texture_from_file(pr::backend::vk::ResourceAllocator& allocator, pr::backend::vk::UploadHeap& upload_heap, const char* filename, bool use_srgb)
{
    ...
    {
        {
            auto const barrier = get_image_memory_barrier(res.image, state_change{resource_state::undefined, resource_state::copy_dest}, VK_IMAGE_ASPECT_COLOR_BIT,
        ...
        //
        {
            auto const barrier = get_image_memory_barrier(res.image, state_change{resource_state::copy_dest, resource_state::shader_resource}, VK_IMAGE_ASPECT_COLOR_BIT,
                                                          img_size.num_mipmaps, img_size.array_size);
