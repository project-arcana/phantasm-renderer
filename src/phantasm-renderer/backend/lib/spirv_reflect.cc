...
static SpvReflectResult IntersectSortedUint32(const uint32_t* p_arr0, size_t arr0_size, const uint32_t* p_arr1, size_t arr1_size, uint32_t** pp_res, size_t* res_size)
{
    *res_size = 0;
...
static SpvReflectResult ParseFunctions(Parser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        p_parser->functions = (Function*)calloc(p_parser->function_count, sizeof(*(p_parser->functions)));
        ...
        for (size_t i = 0; i < p_parser->function_count; ++i)
        {
            ...
            p_func->callee_ptrs = (Function**)calloc(p_func->callee_count, sizeof(*(p_func->callee_ptrs)));
...
static SpvReflectResult ParseDecorations(Parser* p_parser)
{
    for (uint32_t i = 0; i < p_parser->node_count; ++i)
    {
        ...
        if (((uint32_t)p_node->op != (uint32_t)SpvOpDecorate) && ((uint32_t)p_node->op != (uint32_t)SpvOpMemberDecorate)
            && ((uint32_t)p_node->op != (uint32_t)SpvReflectOpDecorateId) && ((uint32_t)p_node->op != (uint32_t)SpvReflectOpDecorateStringGOOGLE)
...
static SpvReflectResult ParseTypes(Parser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    p_module->_internal->type_descriptions
        = (SpvReflectTypeDescription*)calloc(p_module->_internal->type_description_count, sizeof(*(p_module->_internal->type_descriptions)));
...
static SpvReflectResult ParseDescriptorBindings(Parser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    if (p_module->descriptor_binding_count > 0)
    {
        qsort(p_module->descriptor_bindings, p_module->descriptor_binding_count, sizeof(*(p_module->descriptor_bindings)), SortCompareDescriptorBinding);
    }
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(
    Parser* p_parser, SpvReflectShaderModule* p_module, bool is_parent_root, bool is_parent_aos, bool is_parent_rta, SpvReflectBlockVariable* p_var)
{
...
static SpvReflectResult ParseDescriptorBlockVariableUsage(
    Parser* p_parser, SpvReflectShaderModule* p_module, AccessChain* p_access_chain, uint32_t index_index, SpvOp override_op_type, SpvReflectBlockVariable* p_var)
{
    (void)p_parser;
    (void)p_access_chain;
    (void)p_var;
    ...
    case SpvOpTypeStruct:
    {
        ...
        {
            SpvReflectResult result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain, index_index + 1, (SpvOp)INVALID_VALUE, p_member_var);
            if (result != SPV_REFLECT_RESULT_SUCCESS)
            {
                return result;
            }
...
static SpvReflectResult ParseInterfaceVariables(Parser* p_parser, SpvReflectShaderModule* p_module, SpvReflectEntryPoint* p_entry, size_t io_var_count, uint32_t* io_vars)
{
    if (io_var_count == 0)
    {
        return SPV_REFLECT_RESULT_SUCCESS;
    ...
    for (size_t i = 0; i < io_var_count; ++i)
    {
        ...
        SpvReflectResult result = ParseInterfaceVariable(p_parser, &p_type_node->decorations, p_module, p_type, p_var, &has_built_in);
        if (result != SPV_REFLECT_RESULT_SUCCESS)
        {
            return result;
        }
...
static SpvReflectResult TraverseCallGraph(Parser* p_parser, Function* p_func, size_t* p_func_count, uint32_t* p_func_ids, uint32_t depth)
{
...
static SpvReflectResult ParseStaticallyUsedResources(
    Parser* p_parser, SpvReflectShaderModule* p_module, SpvReflectEntryPoint* p_entry, size_t uniform_count, uint32_t* uniforms, size_t push_constant_count, uint32_t* push_constants)
{
    ...
    for (size_t i = 0, j = 0; i < called_function_count; ++i)
    {
        ...
        memcpy(&used_variables[used_variable_count], p_parser->functions[j].accessed_ptrs, p_parser->functions[j].accessed_ptr_count * sizeof(*used_variables));
        used_variable_count += p_parser->functions[j].accessed_ptr_count;
    }
    SafeFree(p_called_functions);
    ...
    //
    SpvReflectResult result0 = IntersectSortedUint32(used_variables, used_variable_count, uniforms, uniform_count, &p_entry->used_uniforms, &used_uniform_count);
...
static SpvReflectResult ParseEntryPoints(Parser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i)
    {
        ...
        result = ParseInterfaceVariables(p_parser, p_module, p_entry_point, interface_variable_count, interface_variables);
        if (result != SPV_REFLECT_RESULT_SUCCESS)
        {
            return result;
        }
        ...
        result = ParseStaticallyUsedResources(p_parser, p_module, p_entry_point, uniform_count, uniforms, push_constant_count, push_constants);
        if (result != SPV_REFLECT_RESULT_SUCCESS)
        {
            return result;
        }
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule* p_module)
{
    ...
    {
        qsort(p_module->descriptor_sets, p_module->descriptor_set_count, sizeof(*(p_module->descriptor_sets)), SortCompareDescriptorSet);
    }
...
SpvReflectResult spvReflectEnumerateEntryPointPushConstantBlocks(const SpvReflectShaderModule* p_module, const char* entry_point, uint32_t* p_count, SpvReflectBlockVariable** pp_blocks)
{
    ...
    {
        bool found = SearchSortedUint32(p_entry->used_push_constants, p_entry->used_push_constant_count, p_module->push_constant_blocks[i].spirv_id);
        if (found)
        {
...
const SpvReflectDescriptorBinding* spvReflectGetDescriptorBinding(const SpvReflectShaderModule* p_module, uint32_t binding_number, uint32_t set_number, SpvReflectResult* p_result)
{
    ...
    {
        *p_result = IsNotNull(p_descriptor) ? SPV_REFLECT_RESULT_SUCCESS
...
const SpvReflectDescriptorSet* spvReflectGetDescriptorSet(const SpvReflectShaderModule* p_module, uint32_t set_number, SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_set) ? SPV_REFLECT_RESULT_SUCCESS
                                     : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
    return p_set;
}
...
const SpvReflectInterfaceVariable* spvReflectGetInputVariableByLocation(const SpvReflectShaderModule* p_module, uint32_t location, SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_var) ? SPV_REFLECT_RESULT_SUCCESS
                                     : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
    return p_var;
}
...
const SpvReflectBlockVariable* spvReflectGetPushConstantBlock(const SpvReflectShaderModule* p_module, uint32_t index, SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_push_constant) ? SPV_REFLECT_RESULT_SUCCESS
                                               : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
