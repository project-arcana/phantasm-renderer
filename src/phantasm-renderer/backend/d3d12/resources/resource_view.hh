...
public:
    void initialize(ID3D12Device& device, D3D12_DESCRIPTOR_HEAP_TYPE type, int num_descriptors, int page_size = 4);

    desc_handle allocate(int num_descriptors)
    {
        auto const res_page = mPageAllocator.allocate(num_descriptors);
        return {res_page};
    }

    void free(desc_handle handle) { mPageAllocator.free(handle.raw); }

public:
    [[nodiscard]] D3D12_CPU_DESCRIPTOR_HANDLE getCpuStart(desc_handle handle) const
    {
        // index = page index * page size
        auto const index = handle.raw * mPageAllocator.get_page_size();
        return D3D12_CPU_DESCRIPTOR_HANDLE{mHeapStartCPU.ptr + index * mDescriptorSize};
    }

    [[nodiscard]] D3D12_GPU_DESCRIPTOR_HANDLE getGpuStart(desc_handle handle) const
    {
        // index = page index * page size
        auto const index = handle.raw * mPageAllocator.get_page_size();
        return D3D12_GPU_DESCRIPTOR_HANDLE{mHeapStartGPU.ptr + index * mDescriptorSize};
    }
